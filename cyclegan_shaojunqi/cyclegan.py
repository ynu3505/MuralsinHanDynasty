...
dataloader = ImageDataset("./monet2photo/" , transform_=transform_, unaligned=True).set_attrs(batch_size=opt.batch_size, shuffle=True, num_workers=opt.n_cpu)

val_dataloader = ImageDataset("./%s" % opt.dataset_name, transform_=transform_, unaligned=True, mode="test").set_attrs(batch_size=5, shuffle=True, num_workers=1)
import cv2
...
for epoch in range(opt.epoch, opt.n_epochs):
    if epoch%40==0:
        G_AB.save(os.path.join(f"./saved_models/G_AB_{epoch}.pkl"))
        G_BA.save(os.path.join(f"./saved_models/G_BA_{epoch}.pkl"))
        D_A.save(os.path.join(f"./saved_models/D_A_{epoch}.pkl"))
        D_B.save(os.path.join(f"./saved_models/D_B_{epoch}.pkl"))
